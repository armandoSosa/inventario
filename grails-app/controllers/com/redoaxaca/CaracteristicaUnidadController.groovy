...
class CaracteristicaUnidadController {
	...
	def save() {
		...
		flash.message = message(code: 'default.created.message', args: [
			message(code: 'caracteristicaUnidad.label', default: 'CaracteristicaUnidad'),
			caracteristicaUnidadInstance.id
		])
	...
	def verificarSiExiste() {
		...
		def caracProbable = Caracteristica.get(Long.parseLong(params.caracteristicaTexto))
		def unidadProbable = Unidad.get(Long.parseLong(params.unidadTexto))
		def caracUnidad = CaracteristicaUnidad.findAllByCaracteristicaAndUnidad(caracProbable, unidadProbable)

		if (caracUnidad) {
			System.out.println("Existe");
			render(controller: "caracteristicaUnidad", template: "verificarExistencia", model: [existe: 1])
			return
		} else {
			System.out.println("NO Existe");
			render(controller: "caracteristicaUnidad", template: "verificarExistencia", model: [existe: 2])
			return
		}
	}
	...
	def editar_caracUnidad(){
		def caracteristicaUnidadInstance = CaracteristicaUnidad.get(Long.parseLong(params.idCU))
		if (!caracteristicaUnidadInstance) {
			flash.message = message(code: 'No se ha encontrado la Característica Unidad')
			redirect(action: "menu")
			return
		}
		caracteristicaUnidadInstance.properties = params
		...
		if (!caracteristicaUnidadInstance.save(flush: true)) {
			render(view: "editar", model: [caracteristicaUnidadInstance: caracteristicaUnidadInstance])
			return
		}

		flash.message = message(code: 'La característica unidad se ha actualizado correctamente')
		redirect(action: "mostrar", id: caracteristicaUnidadInstance.id)
	}
}
